...
extension SCNNode {
    func getPiece(depth: Int) -> Piece? {
        if depth < 0 {
            return nil
        }
        
        if let piece = self as? Piece {
            return piece
        }
        
        return self.parent?.getPiece(depth: depth - 1)
    }
    
    func getBoard(depth: Int) -> Board? {
        if depth < 0 {
            return nil
        }
        
        if let board = self as? Board {
            return board
        }
        
        return self.parent?.getBoard(depth: depth - 1)
    }
}
